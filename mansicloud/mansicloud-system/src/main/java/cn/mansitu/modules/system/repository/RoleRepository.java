 ...
 */
package cn.mansitu.modules.system.repository;
...
public interface RoleRepository extends JpaRepository<Role, Long>, JpaSpecificationExecutor<Role> {
    ...
    /**
     * 根据级别查询(只返回级别大于指定值的角色)
     * @param level 级别值
     * @return /
     */
    @Query(value = "SELECT r.* FROM sys_role r WHERE r.level > ?1 ORDER BY r.level ASC", nativeQuery = true)
    List<Role> findByLevelGreaterThan(int level);
}
