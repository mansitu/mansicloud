 ...
 */
package cn.mansitu.modules.system.service.impl;
...
import cn.mansitu.modules.system.repository.RoleRepository;
import cn.mansitu.modules.system.repository.UserRepository;
import cn.mansitu.modules.system.service.mapstruct.DeptMapper;
import cn.mansitu.utils.*;
import cn.mansitu.utils.enums.DataScopeEnum;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import cn.mansitu.modules.system.domain.Dept;
import cn.mansitu.modules.system.domain.User;
import cn.mansitu.modules.system.service.dto.DeptDto;
import cn.mansitu.modules.system.service.dto.DeptQueryCriteria;
import cn.mansitu.modules.system.service.DeptService;
...
public class DeptServiceImpl implements DeptService {
    ...
    public void update(Dept resources) {
        Dept dbDept = deptRepository.findById(resources.getId())
                .orElseThrow(() -> new BadRequestException("部门不存在"));

        resources.setIsDeleted(dbDept.getIsDeleted());

        Long newPid = resources.getPid();
        if (newPid != null && resources.getId().equals(newPid)) {
            throw new BadRequestException("上级部门不能选择自己");
        }

        Long oldPid = dbDept.getPid();

        resources.setId(dbDept.getId());
...
//            deptRepository.deleteById(deptDto.getId());
            ...
            Dept dept = deptRepository.findById(deptDto.getId()).orElse(null);
            if(dept != null) {
                dept.setIsDeleted(1);
    ...
    // // ==================== 校验公司代号重复 ====================
    // private boolean isCompanyCodeExists(String companyCode) {
    //     return deptRepository.existsByCompanyCodeAndIsDeleted(companyCode, 0);
    // }
    ...
    public void delete(Long id) {
        Dept dept = deptRepository.findById(id)
                .orElseThrow(() -> new BadRequestException("公司不存在"));
        dept.setIsDeleted(1);
        deptRepository.save(dept);
    }
